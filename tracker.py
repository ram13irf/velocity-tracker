...
while True:
    ...
    fps_display = 0.8 * fps_display + 0.2 * (1 / dt)
    prev_time   = now
    ...
    if is_tracking:
        ...
        if ok:
            ...
            prev_center = (cx, cy)
            avg_speed   = float(np.mean(velocities)) if velocities else 0
            ...
            if abs(pred_cx - cx) > 2 or abs(pred_cy - cy) > 2:
                cv2.arrowedLine(frame, (cx, cy), (pred_cx, pred_cy), ORANGE, 2, tipLength=0.4)
    ...
    if key == ord("s"):
        ...
        if new_bbox[2] > 0 and new_bbox[3] > 0:
            small_init  = cv2.resize(sel_frame, (int(w*TRACK_SCALE), int(h*TRACK_SCALE)))
            scaled_bbox = (
